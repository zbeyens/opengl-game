...
void SceneTerrain::Init()
{
	...
	m_pSkybox = (TextureCubemap*)res.LoadResource(ResourceManager::TEXTURECUBEMAP, "skybox");
...
void SceneTerrain::PreRender()
{
	...
	//	glMatrixMode(GL_PROJECTION);
	//	glLoadIdentity();
	//	glOrtho(-tabOrtho[i], tabOrtho[i], -tabOrtho[i] * sizey, tabOrtho[i] * sizey, (GLdouble)cfg.getf("cam_znear"), (GLdouble)cfg.getf("cam_zfar"));
	//	glMatrixMode(GL_MODELVIEW);
	...
	//glMatrixMode(GL_PROJECTION);
	//glLoadIdentity();
	//glOrtho(-1.0, 1.0, -1.0, 1.0, (GLdouble)cfg.getf("cam_znear"), (GLdouble)cfg.getf("cam_zfar"));
	//Frustum::getInstance().Extract(sun_pos);
...
void SceneTerrain::Render()
{
	...
	strStream << cfg.geti("terrain_chunks_drawn") << " chunks";
	DrawString(vec2(1.0f, 20.0f), GLUT_BITMAP_HELVETICA_18, strStream.str());
	strStream.str("");
	strStream << cfg.geti("terrain_chunks_reflected_drawn") << " reflected chunks";
...
void SceneTerrain::RenderWaterSurface()
{
	...
	//	m_pShaderWater->Uniform("win_width", cfg.geti("win_width"));
	//	m_pShaderWater->Uniform("win_height", cfg.geti("win_height"));
	...
	//	float d = 2.0f * cfg.getf("cam_zfar");
...
void SceneTerrain::RenderEnvironment(bool bWaterReflection, bool bDepthMap)
{
	...
	//	if (cam.getEye().y < cfg.getf("water_height"))
	//	{
	//		bWaterReflection = false;
	//		cfg.set("enable_underwater", true);
	//	}
	//	else
	//	{
	//		cfg.set("enable_underwater", false);
	//	}
	...
	//		cam.RenderLookAt(true, cfg.getf("water_height"));
	//	}
	...
	//		m_pShaderTerrain->Uniform("water_height", cfg.getf("water_height"));
	...
	//		cfg.set(bWaterReflection ? "terrain_chunks_reflected_drawn" : "terrain_chunks_drawn", ret);

	//		m_pTerrain->DrawInfinitePlane(Camera::getInstance().getEye(), 2.0f*cfg.getf("cam_zfar"));
	//	}
	//	m_pShaderTerrain->Deactivate();
